...
"""
This module defines the classes relating to get and visualize the 
optical properties based on the dielectronic matrix.
"""
...
class Optics(object):
    ...
    """
    class Optics aiming to calculate the relative properties that 
    ...
    def __init__(self, smear='gaussian', sigma=0.01, *args, **kwargs):

        """
        args::
           sigma: parameter for broading the dielectric matrix;

           smear: type to broad the dielectric matrix, by using 
                   'gaussian' or 'lorentz', default 'gaussian';
        """
    ...
    @property 
    def dieletric_matrix(self):
        pass 

    # method dielectric matrix %
    def get_dielectric_matrix(self, path, correct_gap=0.0, *args, **kwargs):

        """
        Method to get the dielectric matrix, including, 
        ...
        args::
           path: the input source that contained dielectric matrix,
                 supported, 
                         dict{'energy':[], 'real':[], 'imag':[]},
                         json, 'energy', 'real', 'imag';
                         vasp output OUTCAR, EPS, vasprun.xml;

           correct_gap: the corrected or experinmental band gap;
	...
	hw = []           # energy % 
	imag_epsilon = [] # imaginary part % 
	real_epsilon = [] # real part % 

        real = False      # bool, is real_part %  
        imaginary = False # bool, is imaginary_part % 

	# get the hw/real/imaginary epsilon %
        # Attention:: test not efficiency  and just for VASP.5.3 %
        #=======================================================================
        ...
        # efficiency % 
        # for VASP.5.3 and VASP.5.4 % 

	self.__transform__(hw, real_epsilon, imag_epsilon)

    @staticmethod
    def __transform__(hw, real_epsilon, imag_epsilon):
	"""
	To transform the dielectronic constants, with/without broaden
	imaginary part, default is None, and Gaussian/Lorentz type 
        can be used.
	"""
	# transform %     
	epsilon=[]
