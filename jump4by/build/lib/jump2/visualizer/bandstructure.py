...
class PlotBandStructure(BasicPlot):
    ...
    def __init__(self, band=None, *args, **kwargs):
        ...
        from ..analysis import ExtractBand
        
        super(PlotBandStructure, self).__init__()

        if isinstance(band, ExtractBand):
            self.band = band # an object of data extract from 
                             # band structure calculation %      
        else:
            raise ModuleError ("incorrect object %s")

    @staticmethod
    def refine_bands(eigenvalues, y_min, y_max, scale=2.0):
        ...
        eigen = np.eigenvalues.T[0] # reshape the eigenvalue to get eigenvalues
    ...
    def projected_band(num_band, ispin, nelect):
        ...
        if type(num_band) is int:
            ...
            for i in xrange(0, num_band*2):
                if ispin == 1:
                    bands.append(int(nelect/2.0)-num_band+i)
                if ispin == 2:
                    bands.append(int(nelect/1.0)-numband+i)
    ...
    def plot(self, ax, ymin=-4.0, ymax=4.0, edges=True, filled=False, surface_color='m', 
        ...
        if filled is False:
            eigenv = self.redefine_bands(eigenvalues, ymin, ymax, broad=4.0)
            
            count = 0 
            for eigen in eigenv:
                if ispin == 1: 
                    if count > int(nelect/2.0) and gap >= 0.0:
                        eigen += shift
                else:
                    if count > int(nelect) and gap >=0.0:
                        eigen += shift
                ...
                if projected is True and proband:
                    ...
                    if isinstance(nbands, int):
                        bands = self.projected_bands(nbands, ispin, nelect)
        ...
        if filled is True:
            eigenv = self.vc_bands(eigenvalues, ispin, gap, nelect)
            ax.fill_between(kpoints, [ylim]*len(kpoints), eigenv[0], color=surface_color, alpha=0.5)
            ax.fill_between(kpoints, eigenv[1], [ymax]*len(kpoints), color=surface_color, alpha=0.5)

        # show band edges % 
        if edges is True and gap > 0.0:
