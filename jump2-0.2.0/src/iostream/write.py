...
class Write(object):
    ...
    """
    def __init__(self, structure, path, dtype=None):
        """
        Arguments:
    ...
    def run(self, **kwargs):
        ...
        if self.dtype == 'cif':
            ...
            self.__writePOSCAR(coordinate_type, isContainedConstraints)
        elif self.dtype == 'xyz':
            self.__writeXYZ()
        elif self.dtype == 'mol':
            self.__writeMOL()
    ...
    def __writePOSCAR(self, coordinate_type, isContainedConstraints):
        """
        write POSCAR.
        ...
        # check
        if not(coordinate_type.strip().lower().startswith('d')) and not(coordinate_type.strip().lower().startswith('c')):
        ...
        poscar=self.structure.formatting(dtype='poscar', coordinate_type=coordinate_type, isContainedConstraints=isContainedConstraints)
