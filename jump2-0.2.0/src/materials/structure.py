...
from materials.spacegroup import WyckoffSite
...
class Structure(models.Model):
    ...
    例子：
        1.获得结构对象有两个途径：1）读取结构文件；2）查询jump2数据库中的结构。
            ...
            查询jump2数据库中的结构：
                >>> from utils.initialization import initialize_relationships
    ...
    def get_site(self, position, **kwargs):
        ...
        """
        from utils.fetch import get_entity_from_collection
    ...
    def add_atom(self, atom, isUpdatedInfo=False, isPersist=False, **kwargs):
        ...
        """
        import warnings
        from utils.check import exist, check_formated_atom, check_formated_atom_only_direct, check_formated_atom_only_cartesian
    ...
    def _update_related_symmetry_info(self, symprec, angle_tolerance, isPersist=False, **kwargs):
        ...
        import collections
        import spglib
        from utils.fetch import get_symbol_by_z, get_index_from_collection
        from utils.convert import raw2std_position
        from cache.cachedSpacegroupProvider import CachedSpacegroupProvider
        from cache.cachedOperationProvider import CachedOperationProvider
    ...
    def update_by_cell(self, cell, isPersist=False, **kwargs):
        ...
        from utils.check import compare_with_memory
        from utils.fetch import get_atoms_from_cell
    ...
    def _persist(self, **kwargs):
        ...
        参数：
            kwargs:
                ...
                angle_tolerance (default=-1.0):找结构对称性时，控制晶胞基矢之间的角度容差。
        """
        from utils.check import compare_with_db
    ...
    def create(self, raw_structure, isPersist=False, **kwargs):
        ...
        Arguments:
            ...
            kwargs:
                ...
                isConstraints (default=False): whether to write the constrain information to this structure when raw_structure contain them.
        ...
        参数：
            ...
            kwargs:
                isNormalizingCoordinate (default=True):是否移除原子坐标上的平移周期性，以保证其值在0.0～1.0之间。
                isConstraints (default=False):当输入的结构字典数组中包含原子束缚信息时，是否写到结构对象中。
        ...
        isConstraints=False            
        if ('isConstraints' in kwargs) and kwargs['isConstraints']:
            isConstraints=kwargs['isConstraints']
            if isinstance(isConstraints, bool):
                raise ValueError("unrecognized value of 'isConstraints'")
        ...
        for i in xrange(0, len(element_symbols)): # element
            for j in xrange(0, numbers[i]): # number of element
                ...
                if isConstraints and ((not constraints is None) or constraints != []):
    ...
    def formatting(self, dtype='poscar', **kwargs):
        ...
        Arguments:
            ...
            kwargs:
                for 'poscar' type:
                    ...
                    isContainedConstraint: whether contain the constraint information of atomic coordinate. 
        ...
        参数：
            ...
            kwargs:
                for 'poscar' types:
                    ...
                    isContainedConstraint:格式化数组中是否包含原子束缚信息（用于VASP的选择动力学）。
        ...
        if dtype.strip().lower() == 'poscar':
            ...
            # elements, numbers, position and constrain (optional)
            elements=[]
            numbers=[]
            positions=[]
            constraints=[]
            for i in xrange(0, len(self.elements)):
                ...
                for j in xrange(0, numbers[i]):
                    ...
                    if ('isContainedConstraint' in kwargs) and kwargs['isContainedConstraint']:
